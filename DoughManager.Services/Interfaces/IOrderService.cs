...
// Type: DoughManager.Core.Interfaces.IOrderService
...
{
  Task<ServiceResponse<OrderDto>> CreateOrderAsync(OrderDto request, Account account);

  Task<ServiceResponse<OrderDto>> UpdateOrderAsync(OrderDto request, Account account);

  Task<ServiceResponse<List<OrderDto>>> GetAllOrderAsync();

  Task<ServiceResponse<List<OrderDto>>> GetAllPendingOrderAsync();
}
